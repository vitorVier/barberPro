...
import { StatusLegend, STATUS_CONFIG, AppointmentStatusKey } from "./components/status-legend";
          ...
          {appointments.length > 0 && (
            <div className="shrink-0 flex items-center gap-2 px-3 sm:px-4 md:px-5 py-1.5 sm:py-2 border-b border-slate-50 bg-slate-50/50 flex-wrap">
              ...
              {Object.entries(statusCounts).map(([status, count]) => (
                ...
                >
                  {count}{" "}
                  {STATUS_CONFIG[status as AppointmentStatusKey]?.label || status}
                  {Number(count) !== 1 ? "s" : ""}
                </span>
              ))}
            </div>
            ...
            {appointments.length > 0 ? (
              ...
              <div className="flex-1 overflow-y-auto">
                <EmptyState 
                  ...
                  description="Não há agendamentos para este dia. Clique em 'Novo Agendamento' para adicionar."
                />
              </div>
            )}
          </div>
